        ...
        <el-table-column label="操作" width="200">
          <template slot-scope="props">
            <el-button
              size="mini"
              type="success"
              :disabled="props.row.state !== '进行中'"
              @click="handleReceive(props.$index, props.row)">接单</el-button>
            <el-tooltip class="item" effect="dark" content="取消将收取一定费用！" placement="right" :show="props.row.state !== '已完成'">
              <el-button
                size="mini"
                type="warning"
                :disabled="props.row.state === '已完成' || props.row.state === '已送达'|| props.row.state === '已取消'"
                @click="handleCancel(props.$index, props.row)">退单</el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data () {
    return {
      ...
      orderList: [{
        ...
        remark: '无',
        track: {
          placeTime: '2018-12-12 12:12:12',
          payTime: '2018-12-12 12:12:12',
          receiveTime: '2018-12-12 12:12:12',
          deliverTime: '2018-12-12 12:12:12',
          arriveTime: '2018-12-12 12:12:12'
        }
      },
      {
        date: '2018.12.02',
  ...
  mounted () {
    ...
    this.$axios.post('/order/getAllOrderByMerchant',
      {
  ...
  methods: {
    handleReceive (index, row) {
      ...
      let thisVue = this
      this.$axios.post('/order/receiveOrder',
        {
          id: row.id
        })
        .then(function (response) {
          console.log(response.data)
          thisVue.orderList[index].state = '已接单'
        })
        .catch(function (error) {
          console.log(error)
        })
    ...
    handleCancel (index, row) {
      console.log(index, row)
      let thisVue = this
      this.$axios.post('/order/quitOrder',
        {
          id: row.id
        })
        .then(function (response) {
